...
public class EnemyGraph : MonoBehaviour
{
    ...
    public void CreateGraph(bool weightByTower) //argument pour �valuer le chemin en fonction des tours ou pas
    {
        UpdateMap();
        if (mapArray != null)
        {
            ...
            {
                //donn�es pour le calcul de chemins
                ...
                {
                    //si on tombe sur un croisement ou d�but/fin
                    if (
                        mapArray[i, j] == Map.TileType.start
                        || mapArray[i, j] == Map.TileType.end
                        || mapArray[i, j] == Map.TileType.cross
                    )
                    {
                        ...
                        if (mapArray[i, j] == Map.TileType.start) //enregistre l'entr�e
                        {
                        ...
                        if (inPath)
                        {
                            adjDict.Add(
                                previous_node,
                                new List<(Map.coords, int)> { (mapCoords, weightPath) }
                            ); //ajout de l'adjacence
                    ...
                    {
                        //si on sort d'un noeud ou d�but/fin ou si on est d�j� dans un chemin
            ...
            for (int j = 0; j < mapArray.GetLength(0); j++)
            {
                ...
                for (int i = 0; i < mapArray.Length; i++)
                {
                    ...
                    {
                        var mapCoords = new Map.coords(i, j);
                        //pas d'ajout car le noeud est d�j� dans typeDict
                    ...
                    else if (mapArray[i, j] == Map.TileType.road)
                    {
                        ...
                        if (previous_tile_is_node || inPath)
                        {
                            ...
                            if (weightByTower)
                            {
                                //parcours des cases � c�t� voir si il y a une tour construite
                                for (int k = 1; k < 2; k++)
                                {
                                    if (j - k >= 0)
                                    {
